...
def process_file(filename, template):
    ...
    # Extract production, distribution, and consumption solutions from the 'solutions' key
    solutions = enhanced_content.get('solutions', '').split('\n\n')
    production_solutions = solutions[0] if len(solutions) > 0 else ''
    distribution_solutions = solutions[1] if len(solutions) > 1 else ''
    consumption_solutions = solutions[2] if len(solutions) > 2 else ''
    ...
    new_content = template.format(
        ...
        production_solutions=production_solutions,
        distribution_solutions=distribution_solutions,
        consumption_solutions=consumption_solutions,
        **enhanced_content
    )
